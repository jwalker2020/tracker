 ...
 */
export async function scanDemFolder(baseDir: string): Promise<string[]> {
  const entries = await readdir(baseDir, { withFileTypes: true });
  const files: string[] = [];
  for (const e of entries) {
    if (e.isFile() && isGeoTiff(e.name)) files.push(e.name);
  }
  return files.sort();
}
 ...
 */
export async function readFileAsBuffer(absPath: string): Promise<ArrayBuffer> {
  const buf = await readFile(absPath);
  return buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength);
}

export function resolveTilePath(baseDir: string, relativePath: string): string {
