...
function parseSetupNhArgv(argv: string[]): { outputDir?: string; force: boolean } {
...
async function runDownload(argv: string[]): Promise<void> {
  const raw = parseArgv(argv);
  let args: z.infer<typeof DownloadArgsSchema>;
  try {
...
async function runSetupNhCmd(argv: string[]): Promise<void> {
  ...
  try {
    ...
    if (result.failed > 0) console.log("Downloads failed:   ", result.failed);
