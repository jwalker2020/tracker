...
- **Storage**: `gpx_files` stores **summary only**. Full detail is in **`enrichment_artifacts`**: NDJSON file; **each line represents exactly one track**. **`enrichmentArtifactIndex`** on `gpx_files` contains **`{ trackIndex, start, length }`** byte offsets; the API reads a slice using this index. No full-artifact fetch path.
- **Sync vs async**: Sync for small files only; files exceeding ~**15,000 points** or **50 tracks** (`SYNC_MAX_POINTS`, `SYNC_MAX_TRACKS`) are **rerouted to async** (worker). Both use the same artifact-backed storage. Worker is the main production path for large enrichments.
- **Write path**: Enriched tracks streamed to temp NDJSON file; artifact file streamed to PocketBase. **`gpx_files` updated only after artifact persistence succeeds.** Inline `enrichedTracksJson` cleared when artifact is used.
- **Read path**: List/filter/selection use **summary only**. Chart detail: client requests **one track’s slice** via **`GET /api/gpx/files/[id]/enrichment-artifact?trackIndex=N`**. **The artifact API never returns the full artifact file.** Clients must request a single track using `trackIndex`, which allows large artifacts (thousands of tracks) to scale without loading the full artifact into memory. The API returns one track slice only: when the file backend supports HTTP Range requests, only that byte range is fetched; otherwise the server fetches the artifact and slices it in memory. “Elevation profile not available” → missing `hasEnrichmentArtifact`, failed request, parse failure, or not yet loaded.
- **Worker**: Async jobs run in a **separate process** (`pnpm run enrichment-worker`). Worker processes jobs **sequentially** (one at a time per process). Progress and cancel are PocketBase-backed.
...
See **PROJECT_CONTEXT.md** “Debugging enrichment and profiles” for: successful enrichment but missing profiles; artifact persistence failures (logs: `[DEM] Artifact persisted`, `[enrichment-artifact] upload complete`); artifact slice API (Range when supported, else server fetches artifact and slices in memory); missing `hasEnrichmentArtifact`; parse failures; worker/job issues. **Where to look first:** worker logs, browser Network tab (same-origin only), console `[MapView]`.
