...
- **Production deployment (Docker Compose / Coolify):** Approved model documented in `docs/PRODUCTION_DEPLOYMENT.md` and `docs/deployment.md`. **Web** is the only public service (port 3000; Cloudflare Tunnel to https://tracker.nhwalker.net). **Worker** and **PocketBase** are internal-only (no public hostname, tunnel, or exposed port). PocketBase data persisted in volume `pb_data` at `/app/pb_data`. Admin accesses PocketBase via LAN or WireGuard only (SSH port-forward or LAN-restricted host port). `NEXT_PUBLIC_PB_URL=http://pocketbase:8090` for Docker/Coolify; browser does not call PocketBase directly.
- **Worker-based enrichment:** Async enrichment runs only in a **separate worker process or container** (`pnpm run enrichment-worker` locally; in Docker, worker container runs `node --import tsx scripts/enrichment-worker.ts`). Web app creates jobs and returns `jobId`; worker polls for claimable jobs and runs `runEnrichmentJob`. No in-process runner or startup resume in the web app.
...
- **Docs:** PROJECT_CONTEXT.md, CURRENT_STATE.md, EXTERNAL_REVIEW_SUMMARY.md, PRODUCTION_DEPLOYMENT.md, deployment.md, DOCKER_DEPLOYMENT.md aligned with worker architecture, elevation priority, and production deployment model.
